...
-----------------------------
Для начала, создаём корневой сертификат, а к нему уже, будем крепить остальные, но для этого, сначала нужно создать приватный ключ для корневого сертификата.
...
--------------------------------------------------------------------------------------------------------------------------
После генерации закрытого ключа, создаём и подписываем корневой сертификат его же закрытым ключом (делаем самоподписанный сертификат).
