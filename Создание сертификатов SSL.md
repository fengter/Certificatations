...
***Перед созданием сертификатов рекомендуется изучить тему о сертификата, в противном случае, при незнании теории будет бессмысленно проходить практику (не будет погружения в понимание процессов).***
...
1. Корневой сертификат (CA) - это сертификат, который находится в основании дерева сертификатов, он подписывается сторонним центром сертификации (CA), либо самоподписывается.

2. Серверный сертификат - это сертификат, который подписывается корневым сертификатом и устанавливается на сервере.

3. Клиентский (промежуточный) сертификат - это сертификат, который подписывается корневым сертификатом и обеспечивает звено доверия между различными сертификатами, таким образом, что разные производители могут выдавать промежуточные сертификаты, а корневой сертификат у всех может быть один.
...
*Основной командой вызова OpenSSL является - ****openssl***
...
Далее, будут ряд запросов - (Страна, область, город и т.д.), заполняете в соответствии с требованиями.
...
# openssl req -new -newkey rsa:2048 -sha256 -nodes -key (-keyout) clientKey.key (serverKey.key) -out clientReq.csr (serverReq.csr)
-------------------------------------------------------------------------------------------
...
**ВАЖНО** При вводе данных во время создания серверного запроса - внимательно вводите данные в графе - commonName; там нужно указать DNS-имя сервера или его IP-адрес.
---------------------------------------------------------------------------------
...
# openssl x509 -req -in clientReq.csr (serverReq.csr) -CA rootCACert.crt -CAkey rootCAKey.key -CAcreateserial -out clientCert.crt (serverCert.crt) -days 365 -sha256 -extfile client.ext (server.ext)
-------------------------------------------------------------------------------------------------------------------------
...
6. После генерации всех сертификатов, - **clientCert.crt и rootCACert.crt** переносим на другую машину, после чего, устанавливаем их: **rootCACert** - в Корневые доверенные центры сертификации, а **clientCert.crt** - в промежуточные центры сертификации.
