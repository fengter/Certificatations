...
``
Если фактическое DNS-имя машины не соответствует CN сертификата, то работать он не будет, исправить этот недостаток можно присвоив дополнительное DNS-имя к уже существующему в службе DNS (Windows Server).
``

Для этого, открываем ``**"служба DNS -> Зоны прямого просмотра -> имя домена -> ПКМ по окну -> Создать псевдоним (CNAME)"**``
